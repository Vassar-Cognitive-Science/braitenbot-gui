...
Collaborative sessions let multiple people edit one diagram at the same time — the classroom case: an instructor demonstrating, or a lab group co-designing a vehicle and each uploading it to their own robot. Everyone sees the same diagram, the same trace, and the same signal-flow simulation in real time.
...
Guests get a **Follow host** toggle in the Share menu. Turning it on locks your pan and zoom to the host's viewport — useful when the host is walking through a diagram and pointing at things. Panning or zooming manually stops following.
