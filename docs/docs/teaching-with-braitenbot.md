...
robotics: six lessons that walk students from a single sensor wired to a
motor up to a layered, multi-behavior robot, each one built entirely on the
wiring and weights they set themselves. Every lesson runs live in the
browser, so a class can start with zero install — then, if you're building
the hardware, the exact same diagrams upload to a real Arduino-driven robot.
...
Every lesson is a set of live, editable circuits embedded right on the
page — students need nothing installed to work through the whole sequence.
...
assessment ideas — along with an assembly video, are in progress and will
be published here.
:::
